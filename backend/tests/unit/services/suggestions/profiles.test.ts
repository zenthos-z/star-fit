...
describe("recencyConfidence", () => {
  it("decays with age", () => {
    ...
    expect(
      recencyConfidence(new Date(NOW - 5 * 86400000).toISOString(), NOW),
    ).toBe(1);
    expect(
      recencyConfidence(new Date(NOW - 20 * 86400000).toISOString(), NOW),
    ).toBe(0.8);
    expect(
      recencyConfidence(new Date(NOW - 60 * 86400000).toISOString(), NOW),
    ).toBe(0.6);
    expect(
      recencyConfidence(new Date(NOW - 200 * 86400000).toISOString(), NOW),
    ).toBe(0.4);
  });
});
...
describe("computeModifiers", () => {
  it("injury scale from unexpired limitations; expired ignored", () => {
    ...
    const mods = computeModifiers(
      null,
      dynamic as unknown as ProfileDynamic,
      true,
      NOW,
    );
    ...
    const healed = computeModifiers(
      null,
      {
        active_limitations: [dynamic.active_limitations[1]],
      } as unknown as ProfileDynamic,
      true,
      NOW,
    );
  ...
  it("novice caps (fitness_level 已移除，仅按真实历史判断): no-history 0.7 / with-history 0.85", () => {
    expect(computeModifiers({} as ProfileStatic, null, false).novice_cap).toBe(
      0.7,
    );
    expect(computeModifiers({} as ProfileStatic, null, true).novice_cap).toBe(
      0.85,
    );
  });
});
...
describe("buildCapabilityProfile (四级降级)", () => {
  ...
  it("level 2: history — best record when no anchor", () => {
    const history = {
      sessions: [
        {
          date: "2026-08-10T00:00:00Z",
          exercises: [{ name: "杠铃卧推", reps: 5, weight: 85 }],
        },
      ],
    };
    const p = buildCapabilityProfile(
  ...
  it("anchor preferred over history", () => {
    const dynamic = {
      load_anchors: { 杠铃卧推: { best_weight: 60, best_reps: 5 } },
    };
    const history = {
      sessions: [
        {
          date: "2026-08-10",
          exercises: [{ name: "杠铃卧推", reps: 8, weight: 100 }],
        },
      ],
    };
    const p = buildCapabilityProfile(
