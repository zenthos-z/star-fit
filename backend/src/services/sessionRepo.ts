...
export const SessionRepo = {
  ...
  ensureUser: async (
    ...
    if (manualUserId && !isValidUUID(manualUserId)) {
      console.log(
        `[SessionRepo] ensureUser: manualUserId="${manualUserId}" is not a valid UUID, treating as undefined`,
      );
    ...
    console.log(
      `[SessionRepo] ensureUser: deviceId=${deviceId}, manualUserId=${manualUserId ?? "undefined"}, autoCreate=${autoCreate}`,
    );

    // Early return if both deviceId and manualUserId are empty
    if (!deviceId && !manualUserId) {
      console.log(
        `[SessionRepo] ensureUser: No deviceId or manualUserId provided, returning null`,
      );
      return null;
    }
    ...
    if (manualUserId) {
      ...
      await unbindDevice(deviceId); // Ensure device is not taken
      console.log(
        `[SessionRepo] Creating NEW MANUAL user: id=${manualUserId}, device_id=${deviceId}`,
      );
      await client.query(
    ...
    console.log(
      `[SessionRepo] Creating NEW ANONYMOUS user: id=${newId}, device_id=${deviceId}`,
    );
    await client.query(
      "INSERT INTO users (id, device_id, created_at) VALUES ($id, $deviceId, $createdAt)",
    ...
    await client.transaction(
      async (tx) => {
        for (const s of sessions) {
          ...
          await tx.query("DELETE FROM rpe_logs WHERE session_id = $sessionId", {
            sessionId: s.id,
          });
  ...
  getRpeStats: async (
    deviceId: string,
    exerciseName: string,
  ): Promise<{ median: number; count: number; last: number } | null> => {
    ...
    if (cached)
      return cached as { median: number; count: number; last: number };
  ...
  getAllUserSessions: async (
    ...
    const client = SessionRepo.getClient();
    let query =
      "SELECT id, start_time, duration, title, raw_json, ai_audit_text FROM sessions WHERE user_id = $userId ORDER BY start_time DESC";
  ...
  deleteSession: async (sessionId: string): Promise<{ changes: number }> => {
    ...
    console.log(
      `[SessionRepo] Deleted session ${sessionId}, affected rows: ${changes}`,
    );
    return { changes };
  },
  ...
  deleteUser: async (userId: string): Promise<{ success: boolean }> => {
    ...
    await client.transaction(
      async (tx) => {
        // 1. Delete sessions (cascades to rpe_logs)
        ...
        // 3. Delete configs
        await tx.query("DELETE FROM app_configs WHERE user_id = $userId", {
          userId,
        });
        await tx.query(
          "DELETE FROM prompt_style_configs WHERE user_id = $userId",
          { userId },
        );

        // 4. Delete guidance
        await tx.query("DELETE FROM guidance WHERE user_id = $userId", {
          userId,
        });

        // 5. Delete audit_logs
        await tx.query("DELETE FROM audit_logs WHERE user_id = $userId", {
          userId,
        });

        // 6. Delete the user itself
        const u = await tx.query("DELETE FROM users WHERE id = $userId", {
          userId,
        });
    ...
    console.log(
      `[SessionRepo] Deleted user ${userId} and all related data. Total major rows affected: ${changes}`,
    );
