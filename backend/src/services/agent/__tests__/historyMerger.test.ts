...
describe("historyMerger", () => {
  it("returns live rows when summary is empty (sessions table is the source of truth)", () => {
    const live = [
      {
        raw_json: {
  ...
  it("dedupes summary entries already covered by a live row (live wins)", () => {
    ...
    assert.equal(
      (fresh!.exercises as Array<{ name: string }>)[0].name,
      "fresh from sessions table",
    );
  });
