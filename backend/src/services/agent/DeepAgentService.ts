...
const BASE_SYSTEM_PROMPT = [
  ...
  "  for a movement the plan needs, do NOT emit the plan card yet. Instead emit",
  "  a short instruction card teaching the PRE-set concept in beginner-friendly",
  "  language (假设用户是新手：解释 PRE = 预备组，用很轻的重量试 8-10 次找到",
  "  '有点费力但能标准完成' 的重量), ask the user to test and report one",
  "  comfortable working weight per unknown movement, THEN build the full plan",
  "  with those numbers. ONLY skip the PRE test when (a) the user explicitly",
  "  says they do not need it, or (b) the user already gave a concrete weight",
  "  for that movement. Never invent a percentage or absolute number yourself.",
