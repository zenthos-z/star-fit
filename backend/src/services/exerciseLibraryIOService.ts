...
export const ExerciseLibraryIOService = {
  ...
  async exportExercises(
    ...
    if (exportOptions.filterByTarget.length > 0) {
      // 结构化列筛选：命中 primary_muscles 或 secondary_muscles（17 基准词表值）
      exercises = exercises.filter((ex) => {
        const muscles = [
          ...(ex.primary_muscles ?? []),
      ...
      for (const exercise of exercises) {
        if (exercise.assets_json) {
          try {
            ...
            if (assets.video) {
              const videos = Array.isArray(assets.video)
                ? assets.video
                : [assets.video];
              for (const video of videos) {
                // 收集原始视频
                if (video.originalVideoUrl) {
                  const videoPath = this._extractUploadPath(
                  ...
                  if (videoPath) {
                    try {
                      ...
                      zipStructure[
                        `videos/${exercise.id}/${video.id}/original.mp4`
                      ] = buffer;
                      videoCount++;
                      ...
                      if (video.sources) {
                        ...
                        for (const source of video.sources) {
                          if (
                            source.url &&
                            source.quality === exportOptions.videoQuality
                          ) {
                            const qualityPath = this._extractUploadPath(
                              source.url,
                            );
                            if (qualityPath) {
                              try {
                                const qualityBuffer =
                                  await fs.readFile(qualityPath);
                                zipStructure[
                                  `videos/${exercise.id}/${video.id}/${qualityFile}`
                                ] = qualityBuffer;
                              } catch (err) {
                                console.warn(
                                  `[Export] Failed to read quality file: ${qualityPath}`,
                                );
                              }
                            }
                          }
                      ...
                      // 收集封面图
                      if (video.posterUrl) {
                        const posterPath = this._extractUploadPath(
                          video.posterUrl,
                        );
                        if (posterPath) {
                          try {
                            ...
                            zipStructure[
                              `videos/${exercise.id}/${video.id}/poster.jpg`
                            ] = posterBuffer;
                          } catch (err) {
                            console.warn(
                              `[Export] Failed to read poster: ${posterPath}`,
                            );
                          }
                        }
                      }
                      ...
                      console.warn(
                        `[Export] Failed to read video: ${videoPath}`,
                      );
                    }
                  }
                }
            ...
            console.warn(
              `[Export] Failed to parse assets for exercise: ${exercise.id}`,
            );
          }
        }
      }
            ...
            if (assets.cover) {
              ...
              if (coverPath) {
                try {
                  ...
                  zipStructure[`covers/${exercise.id}/cover${ext}`] =
                    coverBuffer;
                  coverCount++;
                } catch (err) {
  ...
  async importExercises(
    ...
    const validationErrors =
      await ExerciseLibraryValidator.validateImportData(extractedData);
    ...
    try {
      // 5. 解决冲突
      console.log(
        "[Import] Resolving conflicts with strategy:",
      ...
      for (const exercise of resolution.toDelete) {
        try {
          ...
          console.error(
            "[Import] Failed to delete exercise:",
            exercise.name,
            e,
          );
          throw e;
      ...
      console.log(
        "[Import] Importing",
        resolution.exercisesToImport.length,
        "exercises...",
      );
      ...
      for (const { exercise, originalName } of resolution.exercisesToImport) {
        try {
          ...
          console.error(
            "[Import] Failed to import exercise:",
            exercise.name,
            error,
          );
          errors.push({
        ...
        for (const { exercise } of resolution.exercisesToImport) {
          if (exercise.assets_json) {
            try {
                ...
                for (const video of videos) {
                  ...
                  if (extractedData[videoKey]) {
                    ...
                    const tempPath = path.join(
                      tempDir,
                      `${video.id}_original.mp4`,
                    );
                    await fs.writeFile(
                      tempPath,
                      extractedData[videoKey] as Buffer,
                    );
              ...
              console.warn(
                `[Import] Failed to process video for exercise: ${exercise.name}`,
                e,
              );
            }
          }
  ...
  async precheckImport(zipBuffer: Buffer): Promise<PrecheckResult> {
    ...
    const extractedData = await ZipHandler.extractArchive(zipBuffer);
    console.log(
      "[PrecheckImport] Extracted files:",
    ...
    if (warnings.length > 0) {
      console.warn(
        `[PrecheckImport] Found ${warnings.length} validation warnings:`,
      );
      warnings.forEach((w) => {
   ...
   * @returns 导入批次列表
   */
  async getUserImports(userId: string): Promise<
    Array<{
      batchId: string;
      status: string;
  ...
  async _failImportBatch(batchId: string, error: unknown): Promise<void> {
    ...
    await client.query(
      ...
      WHERE id = $3
    `,
      [JSON.stringify(errors), new Date().toISOString(), batchId],
    );
  },
};
