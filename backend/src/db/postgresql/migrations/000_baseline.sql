...
-- ============================================================================
-- Purpose: 基线 schema 收编（Core-Flex 三态模型，14 张业务表 + 枚举/索引/触发器/物化视图）。
--   本迁移由 schema.sql 幂等化而来，是全新数据库的唯一基线入口；
--   schema.sql 保留为参考文档，不再由任何运行时路径执行。
--   另前置两类历史断裂依赖：
--   1) fitness_level 枚举：迁移 003 的视图引用 ::fitness_level，但全仓库此前无创建语句；
--   2) pgb_json_merge 函数：迁移 003 文件内视图（先）引用该函数（后定义），干净库上会失败。
--   user_insights 视图不在此建——由 003/004/006 迁移链重建为最终态。
-- Idempotent: 全部语句可重复执行（IF NOT EXISTS / DO 块 / DROP-then-CREATE）。
-- ============================================================================

-- 迁移记录表自举（001 也会建，这里保证 000 自身可记录）
...
-- fitness_level（迁移 003 视图依赖；枚举值与 shared/contracts UserProfileV2 z.enum 对齐）
DO $$ BEGIN
...
-- pgb_json_merge（迁移 003 视图依赖，须先于其视图定义存在）
...
CREATE INDEX IF NOT EXISTS idx_users_profile_static ON users USING gin(profile_static);
CREATE INDEX IF NOT EXISTS idx_users_profile_dynamic ON users USING gin(profile_dynamic);
CREATE INDEX IF NOT EXISTS idx_users_history_summary ON users USING gin(history_summary);

-- Specific path indexes for common queries
CREATE INDEX IF NOT EXISTS idx_users_load_anchors ON users USING gin((profile_dynamic->'load_anchors'));
CREATE INDEX IF NOT EXISTS idx_users_active_limitations ON users USING gin((profile_dynamic->'active_limitations'));
...
-- JSONB GIN indexes
CREATE INDEX IF NOT EXISTS idx_exercises_attributes ON exercises USING gin(attributes);
CREATE INDEX IF NOT EXISTS idx_exercises_targets ON exercises USING gin((attributes->'targets'));
CREATE INDEX IF NOT EXISTS idx_exercises_impact_level ON exercises USING gin((attributes->'impact_level'));
CREATE INDEX IF NOT EXISTS idx_exercises_equipment ON exercises USING gin((attributes->'equipment_required'));
CREATE INDEX IF NOT EXISTS idx_exercises_pattern ON exercises USING gin((attributes->'pattern'));
...
CREATE INDEX IF NOT EXISTS idx_sessions_user_start_time ON sessions(user_id, start_time DESC);

-- JSONB indexes
CREATE INDEX IF NOT EXISTS idx_sessions_raw_json ON sessions USING gin(raw_json);
...
-- RPE logs indexes
CREATE INDEX IF NOT EXISTS idx_rpe_user_exercise ON rpe_logs(user_id, exercise_name);
CREATE INDEX IF NOT EXISTS idx_rpe_session_id ON rpe_logs(session_id);
CREATE INDEX IF NOT EXISTS idx_rpe_timestamp ON rpe_logs(timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_rpe_user_timestamp ON rpe_logs(user_id, timestamp DESC);
...
-- Guidance indexes
CREATE INDEX IF NOT EXISTS idx_guidance_user_id ON guidance(user_id);
CREATE INDEX IF NOT EXISTS idx_guidance_updated_at ON guidance(updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_app_configs_updated_at ON app_configs(updated_at DESC);

-- ============================================================================
-- PROMPT STYLE CONFIGS TABLE - AI System Prompts
-- ============================================================================

CREATE TABLE IF NOT EXISTS prompt_style_configs (
  user_id TEXT NOT NULL DEFAULT 'global',
...
CREATE INDEX IF NOT EXISTS idx_prompt_style_user_id ON prompt_style_configs(user_id);
CREATE INDEX IF NOT EXISTS idx_prompt_style_is_active ON prompt_style_configs(is_active) WHERE is_active = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_deviation_user_id ON deviation_logs(user_id);
CREATE INDEX IF NOT EXISTS idx_deviation_exercise_id ON deviation_logs(exercise_id);
CREATE INDEX IF NOT EXISTS idx_deviation_timestamp ON deviation_logs(timestamp DESC);

-- ============================================================================
-- CACHE: HISTORY SUMMARIES TABLE
-- ============================================================================

CREATE TABLE IF NOT EXISTS cache_history_summaries (
...
CREATE INDEX IF NOT EXISTS idx_cache_history_user_id ON cache_history_summaries(user_id);

-- ============================================================================
-- CACHE: RPE STATS TABLE
-- ============================================================================

CREATE TABLE IF NOT EXISTS cache_rpe_stats (
...
CREATE INDEX IF NOT EXISTS idx_cache_rpe_user_id ON cache_rpe_stats(user_id);
CREATE INDEX IF NOT EXISTS idx_cache_rpe_updated_at ON cache_rpe_stats(updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_audit_field_name ON audit_logs(field_name);
CREATE INDEX IF NOT EXISTS idx_audit_modified_by ON audit_logs(modified_by);
...
CREATE INDEX IF NOT EXISTS idx_video_tasks_exercise_name ON video_tasks(exercise_name);
...
DROP TRIGGER IF EXISTS update_guidance_updated_at ON guidance;
...
DROP TRIGGER IF EXISTS update_prompt_style_configs_updated_at ON prompt_style_configs;
...
CREATE INDEX IF NOT EXISTS idx_mview_user_current_state_id ON user_current_state (id);

-- Exercise summary view for quick lookups
CREATE MATERIALIZED VIEW IF NOT EXISTS exercise_summary AS
SELECT
  e.id,
  e.name,
  e.exercise_type,
...
CREATE INDEX IF NOT EXISTS idx_mview_exercise_summary_id ON exercise_summary (id);
CREATE INDEX IF NOT EXISTS idx_mview_exercise_summary_type ON exercise_summary (exercise_type);
CREATE INDEX IF NOT EXISTS idx_mview_exercise_summary_name ON exercise_summary (name);
