...
<p align="center">
  <img src="docs/screenshots/app-triptych.png" alt="Starfit — 玻璃质感首页 · AI 教练 · AI 海报" width="100%">
</p>

> **左**：Liquid Glass 质感首页　**中**：AI 教练读到你的训练史，随口一问就给出个性化方案　**右**：训练战报一键生成 AI 海报
...
> 大多数 AI 健身 App 把 AI 做成一个聊天框。Starfit 把 AI 做成看不见的教练：
> 它读你的训练史、记住你的伤病、在你需要之前就调整你的计划。
> 你唯一能看到 AI 工作的地方，是它递给你确认的那张卡片。

实际体验：你随口问一句"膝盖不舒服还能练深蹲吗"——
它**不会反问你任何背景问题**，直接查你的训练记录（"最近深蹲 40kg×8×3"）
和恢复状态，给出降载方案和替代动作；对话中识别到伤病信号，
弹一张卡片请你确认记录——确认后，未来 30 天的计划自动避开高膝冲击动作。

三个层次：
| 层次 | 你看到的 | AI 在做的 |
|------|---------|----------|
| 记录时 | 只有动作卡和计时 | 数据自动落库，Agent 随时可查原始数据 |
| 对话时 | 一次点击的确认卡片 | 主动查历史+画像，个性化建议，质量门比对每个数字 |
| 进化时 | 计划悄悄变了 | 伤病自动愈合期、负荷锚点随训练更新 |
